...
ENJOY_URL = "https://aptos-node.souffl3.com/v1"
...
#:!:>section_4
    ...
    def generate_transaction_with_seq(self, sender: str, payload: Dict[str, Any], seq) -> Dict[str, Any]:
        """Generates a transaction request that can be submitted to produce a raw transaction that
        can be signed, which upon being signed can be submitted to the blockchain. """
    ...
    def sign_transaction(self, account_from: Account, txn_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            res = requests.post(f"{self.url}/transactions/encode_submission", json=txn_request, timeout=3)
        except:
            res = requests.post(f"{self.url}/transactions/encode_submission", json=txn_request, timeout=3)
    ...
    def submit_transaction(self, txn: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(f"{self.url}/transactions", headers=headers, json=txn, timeout=3)
        except:
            response = requests.post(f"{self.url}/transactions", headers=headers, json=txn, timeout=3)
    ...
    def execute_transaction_with_payload_with_seq(self, account_from: Account, payload: Dict[str, Any], seq) -> Dict[str, Any]:
        """Execute a transaction for the given payload."""
        
        txn_request = self.generate_transaction_with_seq(account_from.address(), payload, seq)
        signed_txn = self.sign_transaction(account_from, txn_request)
        return self.submit_transaction(signed_txn)
